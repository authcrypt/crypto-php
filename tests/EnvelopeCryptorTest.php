...
final class EnvelopeCryptorTest extends TestCase
{
    ...
    public function testEncryptProducesExpectedStructure(
        ...
        $cryptor = new EnvelopeCryptor(secret: $secret, kdf: $kdf, cipher: $cipher, kwCipher: $kwCipher);
        $result = $cryptor->encrypt($plaintext, $context);
        ...
        // Parse components
        $offset = 0;
        $salt = StringHelper::byteSubstr($result, $offset, $kdfSaltSize);
        $offset += $kdfSaltSize;
        $dekNonce = StringHelper::byteSubstr($result, $offset, $kwNonceSize);
        $offset += $kwNonceSize;
        $parsedWrappedDek = StringHelper::byteSubstr($result, $offset, $dataKeySize + $kwOverheadSize);
        $offset += $dataKeySize + $kwOverheadSize;
        $dataNonce = StringHelper::byteSubstr($result, $offset, $dataNonceSize);
        $offset += $dataNonceSize;
        $encryptedData = StringHelper::byteSubstr($result, $offset);
    ...
    public function testDecryptReturnsPlaintextAndUsesKdfAndCiphers(
        ...
        $cryptor = new EnvelopeCryptor(secret: $secret, kdf: $kdf, cipher: $cipher, kwCipher: $kwCipher);
        $decrypted = $cryptor->decrypt($blob, $context);
    ...
    public function testEncryptionIsRandomized(
        ...
        $res1 = $cryptor->encrypt('data');
        $res2 = $cryptor->encrypt('data');

        $offset = 0;

        // 1. KDF salt (if $kdfSaltSize > 0)
        if ($kdfSaltSize > 0) {
            $salt1 = StringHelper::byteSubstr($res1, $offset, $kdfSaltSize);
            $salt2 = StringHelper::byteSubstr($res2, $offset, $kdfSaltSize);
        ...
        if ($kwNonceSize > 0) {
            $dekNonce1 = StringHelper::byteSubstr($res1, $offset, $kwNonceSize);
            $dekNonce2 = StringHelper::byteSubstr($res2, $offset, $kwNonceSize);
            $this->assertNotSame($dekNonce1, $dekNonce2, 'DEK nonce must be different for each encryption');
        }
        $offset += $kwNonceSize;

        // 3. DEK (must be > 0)
        $dek1 = StringHelper::byteSubstr($res1, $offset, $dataKeySize);
        $dek2 = StringHelper::byteSubstr($res2, $offset, $dataKeySize);
        $this->assertNotSame($dek1, $dek2, 'DEK must be different for each encryption');
        $offset += $dataKeySize + $kwOverheadSize;

        // 4. Data nonce (if $dataNonceSize > 0)
        if ($dataNonceSize > 0) {
            $dataNonce1 = StringHelper::byteSubstr($res1, $offset, $dataNonceSize);
            $dataNonce2 = StringHelper::byteSubstr($res2, $offset, $dataNonceSize);
