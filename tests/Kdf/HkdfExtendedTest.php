...
use Authcrypt\Crypto\Tests\StringableParam;

final class HkdfExtendedTest extends AbstractKdfCase
{
    ...
    public function testDeriveWithHashStaticSalt(): void
    {
        ...
        $kdf1 = new HkdfExtended(hashStaticSalt: $staticSalt);
        $kdf2 = new HkdfExtended(hashStaticSalt: new StringableParam($staticSalt));
        $keySize = 32;
    ...
    {
        $kdf = new HkdfExtended(saltSize: 0);
        $keySize = 32;
        $secret = random_bytes($keySize);
    ...
    {
        $kdf1 = new HkdfExtended(hashStaticSalt: random_bytes(32), saltSize: 0);
        $kdf2 = new HkdfExtended(hashStaticSalt: random_bytes(32), saltSize: 0);
        $keySize = 32;
        $secret = random_bytes($keySize);
    ...
    public function testInvalidStaticSaltThrowsException(): void
    {
        ...
        new HkdfExtended(hashAlgo: 'sha256', hashStaticSalt: random_bytes(31));
    }
    ...
    {
        $kdf = new HkdfExtended(hashAlgo: $hashAlgo, hashStaticSalt: '', saltSize: 0);
    ...
    protected function createKdfInstance(?string $hashAlgo = null): KdfInterface
    {
        return isset($hashAlgo)
            ? new HkdfExtended(hashAlgo: $hashAlgo)
            : new HkdfExtended();
    }
}
